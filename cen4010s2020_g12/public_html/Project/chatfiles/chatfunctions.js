...
var c_subdir ='';  // get the subdir with chatrooms
var chatroom ='';  // store chat room
...
var chats, chatuser, chatusers;  //html elms.
var texts = {};  //texts from $lsite
...
var cookie_namec ='';
var cookie_roomc ='';  // get chat room name from cookie
var chatrooms = {};    // Get <span> with chatroom
var cookie_beepc ='';  //to set playbeep with the value from cookie

//for data stored in cookie
function setCookieData(){
  // If no user added in form, calls to get chat user name from cookie
  cookie_namec = (chatuser && chatuser.value.length>1) ? '0' : GetCookie('name_c');
  ...
  if (cookie_namec!='0' && document.getElementById('name_code')) {
    ...
    chatuser.value = cookie_namec;
    logoutchat = 0;     // set to not delete the user from list
    chatuserset = 1;
  }

  cookie_roomc = GetCookie('room_c');
  if (cookie_roomc!='0') {
    chatrooms = document.getElementById('chatrooms').getElementsByTagName('span'); // Get <span> with chatroom
...
function setChatRoom(room) {
  ...
  chats.innerHTML = texts.loadroom;
  callphp = 0; // set "callphp" to 0 to force next ajax access to php file
...
function addChatS(text) {
  if (chatuserset == 1) {
    ...
    if(chat < 2 || chat > 200) {
      ...
      ajaxF(setchat, send_chat, reAjax);
      text.adchat.value = '';
    }
  }
  else setNameC(text);
...
var ob_aj = (window.XMLHttpRequest) ? new XMLHttpRequest() : new ActiveXObject('Microsoft.XMLHTTP');    // Create the object for AJAX
function ajaxF(file, dsend, reF) {
  ajxsend = 1; // parameter to check sending (Ajax busy)

  dsend +='&chatroom='+chatroom;   // Add the "chatroom"
  // Sends data
  ob_aj.open('POST', file, true);
  ob_aj.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
  ob_aj.send(dsend +'&isajax=1');

  ob_aj.onreadystatechange = stateChanged;

  // Function to display response
  function stateChanged(){
    if(ob_aj.readyState==4){ /// alert(ob_aj.responseText)
      if(ob_aj.status==200){
        // get chat content to be added in page. 
        var obChat = JSON.parse(ob_aj.responseText);
        reF(obChat);
      }
    } /// console.log(ob_aj.responseText);
    ajxsend = 0;  // Set Ajax sent and free
  }
}

// gets the object from Ajax, returns the items in HTML format
function setHtmlChat(obChat) {
  var chatrows = '';   // stores the area with chat lines
  var chatusersli = '';  // stores the area with online users
  var nrchats = obChat.chats.length;

  // if last-added-chat changed, define $chatrooms html; else gets content from #chats tag
  if(obChat.time != lastaddedc) {
    // if chat lines, and first chat line not empty, traverses the array with chat line and sets <p> with each chat line data
    if(nrchats > 0 && obChat.chats[0].chat && obChat.chats[0].chat != '') {
      for(var i=0; i<nrchats; i++) {
        chatrows += '<p><span class="chatusr">&bull; '+ obChat.chats[i].user +' - </span><em>'+ obChat.chats[i].d_t +'</em><span class="chat">- '+ bbcodeParser.bbcodeToHtml(obChat.chats[i].chat) +'</span></p>';
      }
    }
    else chatrows += '<p><span class="chatusr">'+ texts.notchat +'</p>';

    if(chats) chats.innerHTML = chatrows; // update tet chat
  }

  // refreshes online user list every seccond access
  if(chatuser && (refresh_usrs %2) == 0) {
    // adds the users from object in array, and sorts them alphabetically
    var users = [];
    if(chatuserset == 1) users.push(chatuser.value);
    for(var kusr in obChat.users) {
      if(chatuser.value != obChat.users[kusr]) users.push(obChat.users[kusr]);
    }
    ...
    if(users.length > 0) {
      ...
      chatusersli += '<ul id="chatusersli"><li>'+ users.join('</li><li>') +'</li></ul>';
    }
    else chatusersli = texts.no1online;

    // if "logoutchat" is 1, removes the user from Online list
    if(logoutchat === 1) {
      var linethisuser = new RegExp('\<li\>'+chatuser.value+'\<span\>([^\<]*)\<\/span\>\<\/li\>', 'i');
      if(chatusersli.match(linethisuser)) chatusersli = chatusersli.replace(linethisuser, '');
    }
    
    if(chatusers) chatusers.innerHTML = '<h4 id="onl">'+ texts.online +' '+ users.length +'</h4>'+ chatusersli;   // update chat users
  }
  refresh_usrs++;
}

//callback function called in ajax response
function reAjax(obChat){
  if(chats){
    var niv_scroll = chats.scrollTop;  // determine where the scrollbar is positioned
    setHtmlChat(obChat); // set and add chat content in page

    // Make auto-scroll to scrollbar position if it was moved, or bottom of DIV #chats
    var div = chats;
    var scrollHeight = Math.max(div.scrollHeight, div.clientHeight);
    if(niv_scroll!=0 && niv_scroll<scrol0) div.scrollTop = niv_scroll;
    else {
    ...
    if (i_scrol==0) {
      scrol0 = chats.scrollTop;
      i_scrol = 1;
    }

    if(playbeep == 2) playBeep(obChat.time);  // calls to check for play beep if chat added
  }
}
...
function apelAjax() {
  ...
  if(callphp <= 0){
    callphp = 2.8 + (getNrChatUsers() * 0.3); //set to call "setchat" according to number of online users

    // if Ajax free, sends data, else, set free for next auto-call
    var chatusr = (chatuserset==1) ? (chatuser.value) : '';
    if(ajxsend === 0) ajaxF(setchat, 'chatuser='+chatusr, reAjax);
    else ajxsend = 0;
  }
...
if(chatarea){
  var chat_style = chatarea.getAttribute('data-style');
  if(chat_style =='large') chat_style ='<link rel="stylesheet" href="'+ chatfiles +'/chatstyle.css" />';
  else if(chat_style =='mini') chat_style ='<link rel="stylesheet" href="'+ chatfiles +'/chatstyle_mini.css" />';
  else chat_style ='<meta name="viewport" content="width=device-width, initial-scale=1"><link rel="stylesheet" media="only screen and (max-width: 400px)" href="'+ chatfiles +'/chatstyle_mini.css" /><link rel="stylesheet" media="only screen and (min-width: 401px)" href="'+ chatfiles +'/chatstyle.css" />';

  document.querySelector('head').insertAdjacentHTML('beforeend', chat_style);

  //add chat-body and $texts
  ajaxF(setchat, 'chatarea=1', function(obChat){
    chatarea.insertAdjacentHTML('afterbegin', obChat.html);
    texts = obChat.texts;
    c_subdir = document.getElementById('chatwindow').getAttribute('data-subdir');  //subdir with chatrooms
    chatroom = document.getElementById('chatroom') ? document.getElementById('chatroom').value : document.getElementById('s_room').innerHTML;  //chat room
